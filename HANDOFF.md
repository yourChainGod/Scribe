...
6. **Inline Git Blame + Merge Conflict UI (Phase 35c)**：行末
   ...
   35c-ii-α 已交付 RelativeTime helper（6 buckets：just-now
   / minutes / hours / days / months / years，未来 epoch
   coerce 到 just-now，6 i18n keys × 2 locale，9 单元）。
   35c-ii-β 接 GitBlameEngine actor + per-URL [Int:
   BlameLine] cache + Workspace 绑定，35c-ii-γ 接 Scintilla
   `SCI_EOLANNOTATIONSETTEXT` + caret line change hook +
   hover tooltip，35c-iii 加 Settings 开关 + i18n。
...
Swift 6 strict 0/0 · 318 tests + 4 perf budget · ScintillaCodeEditor.swift
1083 → 385 行 · .app 双击即用 · CI workflow（账单暂停中、`gh workflow disable CI` ）· 4 道闸本地 discipline ·
README/ROADMAP/HANDOFF 同步到位。

下一拍：Phase 35c-ii-β (GitBlameEngine actor + Workspace 绑定) 或 35b-4-g (in-place / submodule / 流式 load) 或换主线 (LargeFile v3 / CLI v2 / Document Map …)。
```
